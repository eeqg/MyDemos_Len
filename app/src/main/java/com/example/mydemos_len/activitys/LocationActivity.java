...
public class LocationActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		showLocation = (TextView) findViewById(R.id.tvShowLocation);
	}
	...
	public void location(View view){
		...
		clientOption.setIsNeedAddress(true);// 中文地址
		clientOption.setCoorType("bd09ll");// gcj02 国测局经纬度坐标系 ；bd09 百度墨卡托坐标系；bd09ll 百度经纬度坐标系
		clientOption.setLocationMode(LocationClientOption.LocationMode.Battery_Saving);// 设置定位模式
		clientOption.setScanSpan(5*60000);//检查周期 小于1秒的按1秒
	...
	private BDLocationListener mListener = new BDLocationListener() {
		...
		public void onReceiveLocation(BDLocation location) {
			...
			if (null != location && location.getLocType() != BDLocation.TypeServerError) {
				...
				logMsg(sb.toString());
				Log.d("test_wp","LocationActivity--onReceiveLocation()--"+sb.toString());
			}
		}
		...
		try {
			if (showLocation != null){
				new Thread(new Runnable() {
					@Override
					public void run() {
						showLocation.post(new Runnable() {
							@Override
							public void run() {
